...
const char * const BITCOIN_CONF_FILENAME = "ubiquitium.conf";
...
#else
    const char* pszModule = "ubiquitium";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Ubiquitium";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Ubiquitium";
#else
    // Unix
    return pathRet / ".ubiquitium";
#endif
#endif
}
